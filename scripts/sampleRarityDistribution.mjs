...
// ─── Helpers ───
// 宣稱值與 rarityIndex.js 的 scoreTier 保持同步（2026 校準後基於 3000 樣本實測）
const CLAIMED_RATES = {
  '傳奇級命盤': 1.5,
  '極稀有': 4,
  '非常稀有': 9,
...
TIER_ORDER.forEach((name) => {
  ...
  const pct = ((count / successful) * 100).toFixed(2)
  const claimed = CLAIMED_RATES[name]
  const ratio = claimed ? (parseFloat(pct) / claimed).toFixed(1) : '-'
  const bar = '█'.repeat(Math.round(parseFloat(pct) / 2))
  console.log(`  ${name.padEnd(10, '　')} ${String(count).padStart(5)} (${pct.padStart(6)}%) ${bar}`)
  if (claimed) {
    console.log(`    宣稱 Top ${claimed}% · 實際 ${pct}% · 偏離 ${ratio}×`)
  }
})
