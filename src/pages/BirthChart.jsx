        ...
        <div className="space-y-6">
          {!chart ? (
            ...
            <>
              ...
              {/* ②-c 3 顆快速分享按鈕（病毒鉤子 — 降低分享摩擦） */}
              <div className="glass-panel p-5 bg-white/[0.02]">
                <p className="text-sm text-saffron-400/80 font-serif italic mb-4 text-center">
                  傳給誰？三個快速入口
                </p>
                <div className="grid sm:grid-cols-3 gap-3">
                  ...
                  <a
                    href={(() => {
                      ...
                      if (encoded) params.set('invite', encoded)
                      if (form.gender) {
                        params.set('rel', form.gender === 'other' ? 'friend' : 'romantic')
                      }
                      return `/compatibility?${params.toString()}`
                    })()}
                    onClick={() => trackEvent('quick_share_to_match', { rarity: rarity?.topPercent })}
                    className="rounded-xl border border-saffron-500/30 bg-saffron-500/5 hover:bg-saffron-500/10 p-4 text-center transition group"
                  >
                    <div className="text-2xl mb-2">💞</div>
                    <div className="text-sm font-medium text-slate-100 group-hover:text-saffron-300">
                      傳給 TA 算合盤
                    </div>
                  ...
                  {/* 2. 傳到社群 — native share 快速觸發 */}
                  <button
                    type="button"
                    onClick={async () => {
                      if (typeof navigator === 'undefined' || !navigator.share) {
                        await copyToClipboard(window.location.href)
                        alert('連結已複製，貼到 IG / 限動即可')
                        return
                      }
                      try {
                        await navigator.share({
                          title: rarity
                            ...
                            : '我的吠陀命盤',
                          text: rarity
                            ? `我剛算完我的吠陀命盤 😳 全人口只有 ${rarity.topPercent}% 長這樣。`
                            : '我剛算了我的吠陀命盤',
                          url: window.location.href
                        })
                        trackEvent('quick_share_social', { rarity: rarity?.topPercent })
                      } catch (err) {
                  ...
                  <button
                    type="button"
                    onClick={async () => {
                      const taunt = rarity && rarity.topPercent < 10
                        ? `你絕對猜不到我是 Top ${rarity.topPercent}% 的哪一種 🫠\n快來算 → ${window.location.href}`
                        : `你命盤應該很平均吧？ 🫠 不服就來算 → ${window.location.href}`
                      if (typeof navigator !== 'undefined' && navigator.share) {
                        try {
                          await navigator.share({
                            title: '不服來算',
                          ...
                          trackEvent('quick_share_taunt', { rarity: rarity?.topPercent })
                          return
                        } catch (err) {
                          if (err?.name === 'AbortError') return
                        }
                      }
                      await copyToClipboard(taunt)
                      alert('挑釁文案已複製，貼給最雷的朋友')
                    }}
                    className="rounded-xl border border-saffron-500/30 bg-saffron-500/5 hover:bg-saffron-500/10 p-4 text-center transition group"
                  >
                    <div className="text-2xl mb-2">🫠</div>
