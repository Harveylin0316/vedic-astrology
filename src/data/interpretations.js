...
// ═════════════════════════════════════════════════════════════
// [3-a] Moon Traits — 月亮星座優缺點（給人的相處感受）
// ═════════════════════════════════════════════════════════════
...
// ═════════════════════════════════════════════════════════════
// [3-b] Sun Traits — 太陽星座優缺點（核心人格、職場樣子）
// ═════════════════════════════════════════════════════════════
...
// ═════════════════════════════════════════════════════════════
// [8] Vimshottari Dasha 大運 — Raman 框架，按年齡階段分層事件
//
// Raman 法則：
// - 每個 Graha 有自己的「Karakas（自然徵象）」— 太陽=父親、名聲；
//   月亮=母親、心智；火星=兄弟、房產；水星=智力、生意；木星=智慧、
//   子嗣、財富；金星=妻子/愛情、藝術；土星=紀律、延遲、業力；
//   Rahu=慾望、海外；Ketu=出離、靈性。
// - 大運主題由該 Graha 的 Karakas 主導，但「呈現方式」隨年齡階段
//   不同：童年表現為學業與家庭，青年為戀愛與事業，壯年為家業與責任，
//   老年為傳承與健康。
// - typicalEvents 按 5 個年齡階段分層：
//   child (0-11), teen (12-19), young (20-34), adult (35-54), elder (55+)
// ═════════════════════════════════════════════════════════════
export const dashaReadings = {
  Ketu: {
    ...
    events: {
      child: ['家中有人離世或生重病', '搬家 / 轉學', '特別喜歡獨處', '對神明 / 靈異敏感', '莫名的早熟'],
      teen: ['突然迷上宗教 / 神祕學', '跟原本的朋友圈斷聯', '對主流升學路線產生懷疑', '早期靈性啟蒙'],
      young: ['分手 / 失戀痛徹心扉', '突然離職 / 轉行', '出家 / 長期閉關念頭', '遭遇重大覺醒事件', '放棄追了很久的夢想'],
      adult: ['婚姻重大考驗 / 離婚', '突然的職業大轉向', '親人離世', '深層覺醒、開始靈修', '放下長年的執念'],
      elder: ['進入退休 / 出世階段', '靈性追求成為主軸', '告別舊的物質身份', '健康層面的突發事件']
    },
  ...
  Venus: {
    ...
    years: 20,
    theme: '愛情 · 藝術 · 奢華 · 配偶',
    karaka: 'Kalatra（配偶）· 享樂 · 藝術 · 魅力 · 財富（次要）',
    ...
    events: {
      child: ['異性緣早啟動', '學鋼琴 / 畫畫 / 舞蹈等才藝', '外表可愛討喜', '家中物質豐盛', '喜歡漂亮衣服與玩具'],
      teen: ['初戀來了', '開始打扮變美', '藝術才能展現', '朋友圈擴大、人緣好', '零用錢多'],
      young: ['戀愛 / 同居 / 訂婚 / 結婚', '學藝術、設計、時尚', '買美的東西（名牌、珠寶）', '第一次出國玩', '異性緣爆棚'],
      adult: ['婚姻進入甜蜜期 / 添丁', '買房買車 / 置產', '事業進入美學領域', '副業變創作', '海外長假頻繁'],
      elder: ['享受退休生活', '孫輩的寵愛', '藝術興趣再啟', '家族圓滿團聚', '贈予子女資產']
    },
  ...
  Sun: {
    ...
    events: {
      child: ['跟父親關係定調', '在學校被老師看見', '個性裡的自信萌芽', '學業成績亮眼', '當班長 / 幹部'],
      teen: ['考試名列前茅', '社團當主席 / 負責人', '跟父親張力浮現', '熱情投入某個興趣並被看見'],
      young: ['考公職 / 考上大企業', '第一次升官', '被主管賞識', '公開曝光（上台、出書、採訪）', '跟父親合作或分離'],
      adult: ['成為某領域代表人物', '媒體曝光 / 公開演講', '父親健康大事', '升到管理階層', '出版 / 得獎'],
      elder: ['退休光榮亮相', '父親離世 / 承擔家族長者角色', '健康議題（心臟、眼睛）', '精神權威建立']
    },
  ...
  Moon: {
    ...
    events: {
      child: ['跟母親關係緊密', '家中添弟妹 / 搬家', '情感敏感萌芽', '喜歡回家、依賴家人', '有關於「食物」的早期記憶'],
      teen: ['情緒起伏明顯', '朋友變成「第二個家」', '跟家人因為獨立議題磨合', '喜歡煮飯 / 照顧人'],
      young: ['結婚', '生第一個小孩', '買第一間房', '母親健康變化', '搬家 / 重新布置家'],
      adult: ['添第二胎 / 子女教育', '換大房子', '母親年老', '經營家業', '情感療癒歷程'],
      elder: ['與子女 / 孫輩團聚', '母親離世 / 承擔家母角色', '老家改建', '情緒的人生回顧']
    },
  ...
  Mars: {
    ...
    karaka: 'Bhratru（兄弟）· 勇氣 · 土地 · 衝突 · 手術 · 血',
    ...
    events: {
      child: ['好動、愛跑跳', '跟兄弟姊妹打架頻繁', '擦傷、跌倒、小意外', '熱愛運動或武術', '比同齡小孩早長身體'],
      teen: ['運動校隊 / 比賽得獎', '叛逆期高峰', '跟同學衝突 / 打架', '熱血愛打電動', '身體發育明顯'],
      young: ['第一次創業', '買第一間房', '熱戀、激情戀愛', '運動受傷 / 小手術', '軍警役', '開始健身', '跟兄弟吵大架'],
      adult: ['投資不動產', '事業衝刺階段', '手術 / 意外住院', '跟同事或同業競爭', '買車 / 搬家', '脾氣家庭戰爭'],
      elder: ['重大手術機會', '血壓 / 心血管議題', '兄弟姊妹關係變化', '賣房或處分不動產']
    },
  ...
  Rahu: {
    ...
    events: {
      child: ['家族有不尋常事', '早熟、大人話題特別懂', '對外國或科技早感興趣', '家庭變化大', '想像力豐富'],
      teen: ['叛逆期爆炸', '迷上網路 / 遊戲 / 外國文化', '身份認同探索期', '非典型興趣'],
      young: ['第一次出國 / 移民', '網紅 / 直播爆紅', '加密貨幣 / 新創淘金', '非典型戀情', '身份戲劇化翻轉', '社群爆紅'],
      adult: ['人生大翻轉', '海外事業大爆發', '離婚 / 再婚', '名聲戲劇性起落', '顛覆既有身份'],
      elder: ['退到非主流路線', '神祕學 / 新興宗教', '晚年海外移居', '健康怪病']
    },
  ...
  Jupiter: {
    ...
    years: 16,
    theme: '智慧 · 子嗣 · 財富 · 導師',
    karaka: 'Guru（導師）· 子嗣 · 智慧 · 法（Dharma）· 財富',
    ...
    events: {
      child: ['受長輩疼愛', '學業順遂', '喜歡看書 / 求知慾強', '有精神啟蒙的老師', '家族聚會多、有貴人'],
      teen: ['考上名校', '出國交換', '人生觀啟蒙', '遇到改變一生的老師', '家境改善'],
      young: ['結婚', '生第一個小孩', '進修 / 讀研究所', '出國念書 / 工作', '創業擴張', '遇到人生貴人'],
      adult: ['子女成就亮眼', '事業巔峰 / 當導師', '出書 / 升博士', '擴大投資', '當顧問 / 專家', '家族財富累積'],
      elder: ['精神追求 / 修行', '寫回憶錄 / 傳承智慧', '祝福晚輩 / 當家族長', '健康（肝、糖）注意']
    },
  ...
  Saturn: {
    ...
    years: 19,
    theme: '紀律 · 責任 · 延遲 · 業力',
    karaka: 'Karma（業）· 延遲 · 紀律 · 職責 · 服務大眾 · 老年',
    ...
    events: {
      child: ['家境嚴峻 / 家中有病人', '被嚴格要求、早熟', '親人離世 / 家庭重擔', '感受孤單 / 不被看見', '學業壓力大'],
      teen: ['升學受挫或重考', '內向、孤僻期', '家庭經濟壓力', '對未來擔憂', '初戀失敗 / 被拒絕'],
      young: ['工作被壓榨', '感情延遲 / 被拒絕 / 長單身', '父母擔子加重', '職場基層熬', '等到快放棄時才成', '穩定買房'],
      adult: ['熬到升遷 / 位置穩定', '父母年老需照顧', '晚婚 / 大年齡差對象', '健康出警訊', '買地 / 蓋房'],
      elder: ['退休後安定', '骨關節、慢性病', '面對死亡議題', '傳承責任', '成為家族的支柱']
    },
  ...
  Mercury: {
    ...
    years: 17,
    theme: '學習 · 溝通 · 生意 · 人脈',
    karaka: 'Budhi（理智）· 語言 · 商業 · 親戚 · 文書 · 年輕感',
    ...
    events: {
      child: ['表現出學習天賦', '愛看書 / 愛問問題', '擅長語言或數學', '跟親戚互動多', '會說話逗大人笑'],
      teen: ['考試表現亮眼', '交筆友 / 網友', '加入文藝 / 辯論 / 營隊', '賺第一筆小錢（家教、打工）'],
      young: ['大學畢業 / 第一份工作', '創業 / 接 freelance', '出書 / 開 YouTube', '人脈圈翻倍', '買賣 / 投資股票'],
      adult: ['事業擴張 / 多元收入', '當講師 / 顧問', '出國做生意', '加倍人脈', '跟親戚合作或分家'],
      elder: ['寫回憶錄 / 口述歷史', '當長輩型導師', '神經系統 / 失眠注意', '晚年仍活躍於學習']
    },
...
// 根據年齡挑選合適的大運事件（Raman 法則：karaka 不變，呈現形式隨年齡轉換）
export function getDashaEventsForAge(lord, startAge, endAge) {
  const reading = dashaReadings[lord]
  if (!reading || !reading.events) return []
  const stages = []
  if (startAge < 12 && endAge >= 0) stages.push('child')
  if (startAge < 20 && endAge >= 12) stages.push('teen')
  if (startAge < 35 && endAge >= 20) stages.push('young')
  if (startAge < 55 && endAge >= 35) stages.push('adult')
  if (endAge >= 55) stages.push('elder')
  // Dedup + collect
  const collected = []
  stages.forEach((s) => {
    ;(reading.events[s] || []).forEach((e) => {
